...
class TestCorrectness:
    def test_correctness_against_actual_ln_forward(self, tiny_model, tokens_and_caches):
        ...
        We use logit difference (correct_token - incorrect_token at the last position) as the
        metric.  A difference cancels the centering offset introduced by process_weights_(), so
        the metric has real signal and the approximation error is tight enough to test with
        atol=1e-3 rather than the loose 0.15 that a sum-based metric requires.
        """
        _, corrupted_tokens, clean_cache, corrupted_cache = tokens_and_caches
        src_layer, src_head = 0, 0
        dst_layer, dst_head = 2, 1

        # Two arbitrary but fixed token indices used as "correct" / "incorrect" answer.
        correct_tok, incorrect_tok = 7, 13

        def logit_diff(logits):
            """Logit difference at the last token position — invariant to center_unembed."""
        ...
        # is the first-order Taylor of the actual LN forward.  The logit-diff metric
        # cancels the centering offset from center_unembed, giving a tight tolerance.
        assert abs(our_metric - ref_metric) < 1e-3, (
            f"Our approx {our_metric:.6f} disagrees with actual-LN reference {ref_metric:.6f} "
            f"(diff={abs(our_metric - ref_metric):.2e}). Possible implementation bug."
        )
...
# ---------------------------------------------------------------------------
# _check_fold_ln: covers both HookedTransformer (.w) and bridge (.weight) paths
# ---------------------------------------------------------------------------


class TestCheckFoldLn:
    """Unit tests for the _check_fold_ln guard."""

    def _make_mock_model(self, attr: str, value):
        """Build a minimal mock that looks like a model with blocks[0].ln1.<attr>."""
        ln1 = types.SimpleNamespace(**{attr: value})
        block = types.SimpleNamespace(ln1=ln1)
        model = types.SimpleNamespace(blocks=[block])
        return model

    def test_no_warning_when_w_is_ones(self):
        """HookedTransformer path: .w == ones → no warning (fold_ln applied)."""
        w = torch.ones(64)
        model = self._make_mock_model("w", w)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            _check_fold_ln(model)  # type: ignore[arg-type]
        assert not caught, "Expected no warning when .w == ones"

    def test_warning_when_w_not_ones(self):
        """HookedTransformer path: .w != ones → UserWarning about unfold_ln."""
        w = torch.full((64,), 2.0)
        model = self._make_mock_model("w", w)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
        ...
        assert "fold" in str(caught[0].message).lower()

    def test_no_warning_when_weight_is_ones(self):
        """TransformerBridge path: .weight == ones → no warning (fold_ln applied)."""
        weight = torch.ones(64)
        model = self._make_mock_model("weight", weight)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            _check_fold_ln(model)  # type: ignore[arg-type]
        assert not caught, "Expected no warning when .weight == ones"

    def test_warning_when_weight_not_ones(self):
        """TransformerBridge path: .weight != ones → UserWarning about unfold_ln."""
        weight = torch.full((64,), 3.0)
        model = self._make_mock_model("weight", weight)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            _check_fold_ln(model)  # type: ignore[arg-type]
    ...
    def test_no_crash_when_blocks_missing(self):
        """Non-standard model with no .blocks → _check_fold_ln silently passes."""
